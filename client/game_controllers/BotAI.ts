...
import { Player, GameState, BotDecision, GameAction } from './types';
...
export class BotAI {
     ...
     */
    private static evaluatePreFlopStrength(holeCards: string[]): number {
        ...
        // Premium pairs
        if (isPair && highCard >= 13) return 95; // AA, KK
        if (isPair && highCard >= 11) return 85; // QQ, JJ
        if (isPair && highCard >= 9) return 75;  // TT, 99
        if (isPair) return 60 + highCard * 2;    // Other pairs

        // Premium non-pairs
        if (highCard === 14 && lowCard >= 12) return 90; // AK, AQ
        if (highCard === 14 && lowCard >= 10) return suited ? 80 : 70; // AJ, AT
        if (highCard === 13 && lowCard >= 11) return suited ? 75 : 65; // KQ, KJ

        // Suited connectors and high cards
        if (suited && Math.abs(val1 - val2) === 1) return 60; // Suited connectors
        if (suited && highCard >= 11) return 55; // Suited with face card
        if (highCard >= 12) return 50; // Any face card

        // Connected cards
        if (Math.abs(val1 - val2) === 1) return 45;
     ...
     */
    private static convertHandRankToStrength(rank: number, description: string): number {
        // pokersolver ranks: 1 (high card) to 9 (straight flush/royal flush)
        const rankStrength: { [key: number]: number } = {
            1: 20,  // High Card
            2: 35,  // Pair
            3: 50,  // Two Pair
     ...
     */
    private static getPositionFactor(botId: string, gameState: GameState): number {
        ...
        if (dealerIndex === -1 || botIndex === -1) return 0.5;
     ...
     */
    private static calculatePotOdds(callAmount: number, pot: number): number {
        if (pot === 0) return 1;
     ...
     */
    private static decideWhenNoBet(bot: Player, gameState: GameState): BotDecision {
        const handStrength = this.evaluateHandStrength(
            bot.hand || [],
            gameState.communityCards,
            gameState.round
        ...
        if (handStrength >= 85) {
            if (random > 0.2) { // 80% bet
                const betAmount = Math.min(
                    Math.floor(gameState.pot * 0.75 + gameState.currentBet),
        ...
        if (handStrength >= 65) {
            if (random > 0.5) { // 50% bet
                const betAmount = Math.min(
                    Math.floor(gameState.pot * 0.5 + gameState.currentBet),
                    bot.chips + bot.currentBet
                );
        ...
        if (random > 0.85) { // 15% bluff bet
            const betAmount = Math.min(
                Math.floor(gameState.pot * 0.3 + gameState.currentBet),
                bot.chips + bot.currentBet
            );
    ...
    private static makeDecision(
        ...
        if (handStrength >= 65) {
            if (random > 0.7 && position > 0.6) { // 30% raise with good position
                const raiseAmount = Math.min(
                    gameState.currentBet + Math.floor(gameState.pot * 0.3),
                    bot.currentBet + chipsLeft
                );
