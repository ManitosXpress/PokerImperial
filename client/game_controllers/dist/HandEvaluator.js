...
export class HandEvaluator {
    ...
    static calculatePots(players) {
        ...
        const sidePots = pots.slice(1).map((pot, idx) => ({
            amount: pot.amount,
     ...
     */
    static determineWinners(players, communityCards, potStructure, rakePercentage = 0.10) {
        const handResults = this.evaluateHands(players, communityCards);
        const winners = [];
        // Helper function to find winners for a specific pot
        const findWinnersForPot = (eligiblePlayerIds, potAmount, potType) => {
     ...
     */
    static evaluateSimpleWinner(players, communityCards, totalPot, rakePercentage = 0.10) {
        const handResults = this.evaluateHands(players, communityCards);
